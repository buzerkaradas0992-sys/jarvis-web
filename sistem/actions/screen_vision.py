...
PLATFORM_NAME = "Windows" if IS_WIN else "macOS"


def _screen_permission_message() -> str:
    if IS_WIN:
        return (
            "Ekran goruntusu alinamadi. Bazi uygulamalar (DRM korumali video "
            "oynaticilar, bazi banka uygulamalari) ekran yakalamayi engeller. "
            "Farkli bir pencere acikken tekrar dene."
        )
    return (
...
# ═══════════════════════════════════════════════════════════════════════════
# Windows — aktif pencere yakalama
# ═══════════════════════════════════════════════════════════════════════════
_dpi_ready = False
...
def _ensure_dpi_awareness() -> None:
    """
    Pencere koordinatlarinin fiziksel piksellerle ortusmesi icin surecin
    DPI farkindaligini acar. Zaten ayarliysa sessizce gecer.
    """
    global _dpi_ready
    if _dpi_ready or not IS_WIN:
        return
    _dpi_ready = True
    try:
        import ctypes
...
def _active_window_info_win() -> tuple[int, str, str, tuple[int, int, int, int] | None]:
    """Aktif pencerenin (hwnd, uygulama adi, baslik, sinirlar) bilgisini dondurur."""
    import ctypes
    ...
    try:
        ...
        if pid:
            ...
            owner = name[:-4] if name.lower().endswith(".exe") else name
    except Exception:
        owner = ""

    # DWM genisletilmis cerceve sinirlari, GetWindowRect'ten daha dogrudur:
    # pencere golgesini disarida birakir.
    bounds = None
    try:
        rect = wintypes.RECT()
    ...
    return hwnd, owner, title, bounds
...
def _grab_region(left: int, top: int, right: int, bottom: int) -> Image.Image:
    """Belirtilen ekran bolgesini PIL goruntusu olarak yakalar."""
    width = right - left
    height = bottom - top
...
def _capture_active_window_win() -> tuple[bool, str, dict | None]:
    _ensure_dpi_awareness()

    try:
        import win32gui  # noqa: F401
    except ImportError:
        return False, "pywin32 yuklu degil. 'pip install pywin32' calistir.", None

    try:
        hwnd, owner, title, bounds = _active_window_info_win()
    except Exception as exc:
        return False, f"Aktif pencere bilgisi alinamadi: {exc}", None

    screen_left, screen_top, screen_right, screen_bottom = _virtual_screen_rect()
    ...
    if use_full_screen:
        ...
        detail = "Aktif pencere yakalanamadi, tum ekran alindi."
    else:
        left, top, right, bottom = bounds  # type: ignore[misc]
        # Ekran disina tasan kisimlari kirp
        ...
        if right - left < 40 or bottom - top < 40:
            left, top, right, bottom = screen_left, screen_top, screen_right, screen_bottom
            detail = "Pencere ekran disinda kaldi, tum ekran alindi."
    ...
    try:
        ...
        return False, f"Ekran goruntusu kaydedilemedi: {exc}", None
...
def _capture_active_window() -> tuple[bool, str, dict | None]:
    """Platforma gore aktif pencereyi yakalar → (ok, hata_mesaji, payload)."""
    if IS_WIN:
        return _capture_active_window_win()
    ...
    if not ok:
        ...
        if "permission" in low or "screen recording" in low:
            return False, _screen_permission_message(), None
        return False, f"Ekran goruntusu alinamadi: {raw}", None

    return _parse_capture_payload(raw)


# ═══════════════════════════════════════════════════════════════════════════
# Gemini vision analizi — platformdan bagimsiz
# ═══════════════════════════════════════════════════════════════════════════
...
def _vision_prompt(query: str, owner_name: str, window_title: str) -> str:
    ...
    return (
        f"Sen {PLATFORM_NAME} uzerinde JARVIS icin ekran analizi yapan bir goruntu yorumlayicisisin.\n"
...
def analyze_screen(query: str, target: str = "active_window") -> str:
    ...
    parsed_ok, detail, payload = _capture_active_window()
    if not parsed_ok:
        return detail
    ...
    try:
        ...
        if image_path.stat().st_size <= 0:
            return (
                "Ekran goruntusu bos geldi. " + _screen_permission_message()
            )
        if _image_looks_blank(image_path):
            return (
                "Ekran goruntusu siyah veya bos gorunuyor. " + _screen_permission_message()
            )
        try:
