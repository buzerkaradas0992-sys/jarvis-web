...
macOS   → Apple Reminders (takvimle ayni EventKit yardimcisi)
Windows → actions/win_organizer.py (Outlook Gorevleri veya JARVIS yerel deposu)

Disariya acilan fonksiyonlar ve dondurulen metinler iki platformda aynidir.
"""
...
def _permission_message() -> str:
    if IS_WIN:
        return (
            "Animsaticilara erisilemedi. JARVIS kendi yerel listesini kullanir; "
            "Outlook gorevlerini kullanmak istersen Outlook'u acip bir hesap "
            "tanimla, sonra tekrar dene."
        )
    return (
...
def get_reminders(query: str = "upcoming", limit: int = 8, list_name: str = "") -> str:
    ...
    if IS_WIN:
        from actions import win_organizer

        ok, detail, reminders = win_organizer.fetch_reminders(mode, limit, (list_name or "").strip())
        if not ok:
...
def add_reminder(
    ...
    if IS_WIN:
        ...
        ok, detail, created = win_organizer.create_reminder(payload)
        if not ok or created is None:
            return f"Animsatici eklenemedi: {detail}"
    else:
        ok, raw = _run_helper("create_reminder", payload=payload, timeout=25)
        if not ok:
            detail = raw.lower()
            if "permission_denied" in detail or "not authorized" in detail or "mach error 4099" in detail:
                return _permission_message()
